...
const Music: React.FC<PageProps> = ({ location }) => {
  const trackMusicLink = (name: string): void => {
    count({ path: `music: ${name}`, title: name, event: true });
  };
  return (
    <Layout>
      <SEO
      ...
      <p>
        ...
        <TextLink
          href={bandcamp}
          onClick={(): void => trackMusicLink('Bandcamp')}>
          my bandcamp page
        </TextLink>{' '}
        and all my videos can be watched on{' '}
        <TextLink
          href={youtube}
          onClick={(): void => trackMusicLink('YouTube')}>
          my YouTube channel
        </TextLink>
        .
      </p>
